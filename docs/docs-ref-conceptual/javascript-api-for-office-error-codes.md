...
|3010|Opération de liaison non prise en charge|Le contenu sélectionné doit être au format Tableau. Organisez les données sous forme de tableau et réessayez.|Le développeur tente d’utiliser la méthode **addRowsAsynch** ou **deleteAllDataValuesAsynch** de l’objet **TableBinding** sur les données du type de forçage _matrice_.|
...
|12002|||Un des éléments suivants :<br> - Aucune page n’existe à l’URL qui a été transmise à `displayDialogAsync`.<br> - La page qui a été transmise à `displayDialogAsync` a été chargée, mais la boîte de dialogue a été redirigée vers une page introuvable ou impossible à charger, ou a été redirigée vers une URL dont la syntaxe n’est pas valide. Levée dans la boîte de dialogue et trigges un `DialogEventReceived` événement dans la page hôte.|
|12003|||La boîte de dialogue a été redirigée vers une URL avec le protocole HTTP. HTTPS est requis. Levée dans la boîte de dialogue et trigges un `DialogEventReceived` événement dans la page hôte.|
|12004|||Le domaine de l’URL est transmise à `displayDialogAsync` n’est pas approuvé. Le domaine doit être le même domaine que la page hôte (y compris le numéro de port et le protocole). Levée par l’appel de `displayDialogAsync`.|
|12005|||L’URL est transmis à `displayDialogAsync` utilise le protocole HTTP. HTTPS est requis. Levée par l’appel de `displayDialogAsync`. (Dans certaines versions de Microsoft Office, le message d’erreur renvoyé avec 12005 est le même un retournées pour 12004.)|
|12006|||La boîte de dialogue a été fermée, généralement parce que l’utilisateur choisit le bouton **X**. Levée dans la boîte de dialogue et trigges un `DialogEventReceived` événement dans la page hôte.|
|12007|||Une boîte de dialogue est déjà ouvert à partir de cette fenêtre hôte. Une fenêtre de l’hôte, par exemple un volet Office, peut uniquement ouvrir une boîte de dialogue à la fois. Levée par l’appel de `displayDialogAsync`.|
|13000 - 13010|||Voir [les Causes et la gestion des erreurs à partir de getAccessTokenAsync](https://docs.microsoft.com/office/dev/add-ins/develop/troubleshoot-sso-in-office-add-ins#causes-and-handling-of-errors-from-getaccesstokenasync).|
...
|:-----|:-----|:-----|
|Matrix|Plage de cellules (y compris dans un tableau et une cellule unique)|Une liaison de type _matrice_ est créée dans les cellules sélectionnées. Aucune modification dans le document n’est attendue.|
|Matrix|Texte sélectionné dans la cellule|Une liaison de type _matrice_ est créée dans la cellule entière. Aucune modification dans le document n’est attendue.|
|Matrix|Sélection multiple/sélection incorrecte (par exemple, l’utilisateur sélectionne une image, un objet ou un objet Word Art.)|Impossible de créer la liaison.|
...
|**Type de liaison spécifié**|**Sélection réelle**|**Comportement**|
|:-----|:-----|:-----|
|Matrix|Texte|Impossible de créer la liaison.|
|Matrix|Tableau entier|Une liaison de type  _matrice_ est créée.Le document est modifié et un contrôle de contenu doit encapsuler le tableau. |
|Matrix|Plage dans un tableau|Impossible de créer la liaison.|
|Matrix|Sélection non valide (par exemple, objets multiples, incorrects, etc.)|Impossible de créer la liaison.|
|Table|Texte|Impossible de créer la liaison.|
|Table|Tableau entier|Une liaison de type  _texte_ est créée.|
|Table|Plage dans un tableau|Impossible de créer la liaison.|
|Table|Sélection non valide (par exemple, objets multiples, incorrects, etc.)|Impossible de créer la liaison.|
